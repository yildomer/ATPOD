"""
Difference -> multiple directions for counting
"""
...
output_video = cv2.VideoWriter('people-counted.avi',
                         cv2.VideoWriter_fourcc(*'MJPG'),
                         10, (1280,720))

mask = cv2.imread("mask.png")

limits_first = [168,394,435,315] # Limits of first line
limits_second = [565,507,757,430] # Limits of second line
...
while True:
    ...
    # Inserting
    image = cvzone.overlayPNG(image,imageGraphics,(730,250))
    ...
    cv2.line(image,(limits_first[0],limits_first[1]),(limits_first[2],limits_first[3]),color = (0,0,255), thickness = 3)
    cv2.line(image, (limits_second[0], limits_second[1]), (limits_second[2], limits_second[3]), color=(0, 0, 255), thickness=3)
    ...
    for result in resultsTracker:
        ...
        if limits_first[0] < cx < limits_first[2] and limits_first[3] - 15  < cy < limits_first[1] + 15:
            if not id in counted_ids_up:
            # if counted_ids.count(id) == 0: # (Alternative)
                counted_ids_up.append(id)
                cv2.line(image, (limits_first[0], limits_first[1]), (limits_first[2], limits_first[3]), color=(0, 255, 0), thickness=3)

        if limits_second[0] < cx < limits_second[2] and limits_second[3] - 15  < cy < limits_second[1] + 15:
            if not id in counted_ids_down:
            # if counted_ids.count(id) == 0: # (Alternative)
                counted_ids_down.append(id)
                cv2.line(image, (limits_second[0], limits_second[1]), (limits_second[2], limits_second[3]), color=(0, 255, 0), thickness=3)


    # Displaying counter.
    # cvzone.putTextRect(image,f'Count: {len(counted_ids)}',(50,50),1,1) # Show counter.
    cv2.putText(image,f'{len(counted_ids_up)}',(915,335),cv2.FONT_HERSHEY_PLAIN,5,color=(93,193,151),thickness=10)
    cv2.putText(image,f'{len(counted_ids_down)}',(1175,335),cv2.FONT_HERSHEY_PLAIN,5,color=(93, 98, 236), thickness=10)
